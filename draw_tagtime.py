...
def save_figure(tags, output):
    ...
    if way == "tag_name":
        y = list()
        if args.tag_name == "messages":
            for t in x:
                y.append(0)
            for peer in data[args.tag_name].values():
                for i in range(len(x)):
                    if peer[i] == "-":
                        continue
                    y[i] += peer[i]
            ones = np.ones(6)/6
            y_ = np.convolve(y, ones, "same").tolist()
            ax.plot(x, y_)
            y_axis = "the number of messages"
        ...
        ax.set_title(y_axis)
        output = output + "_" + args.tag_name + ".eps"
           ...
           # correlation coefficient
            ...
            for t in results.items():
                ...
                for s in results.items():
                    if s[0] == "M":
                        continue
                    cor += (str(np.corrcoef(t[1][way], s[1][way])[0,1]) + " ")
                cor += ("\n")
            c = open(output+way+"_cor.txt", "w")
            c.write(cor)
            ...
            output = output + "_" + args.tag_name + "_std.eps"
...
output = "/home/tktbtk/Data/global_use/"+file_name
